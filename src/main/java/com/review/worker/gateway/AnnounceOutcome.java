 ...
 * because an operator flipped the Gateway's kill switch off. {@code 400}/{@code 409}/{@code 422} are
 * {@link AnnounceStatus#REJECTED_FATAL} — a genuine, non-self-healing Worker-side misconfiguration
 * ({@code backend.id}/{@code worker.id}/{@code llama.model} failing the Gateway's bean validation,
 * {@code backend.id} already owned by a different {@code worker.id}, or {@code backend.url} rejected by
 * the Gateway's host allowlist / not a bare origin).
 */
